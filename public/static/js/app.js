...
app.controller('user',['$scope','$http', function ($scope,$http) {
	$http.get("http://localhost:8080/info-user").success(function (data) {
		$scope.infos = data;
	});
}]);
...
app.controller('tweets-media', ['$scope', '$http', function ($scope, $http) {
    $http.get("http://localhost:8080/tweets-media?page=1").success(function (data) {
    	$scope.tweets = data;
    	console.log($scope.tweets.length);
    	var page = 2;
    	$scope.busy = false;
		$scope.loadTweets = function() {
			...
			$scope.busy = true;
			$http.get("http://localhost:8080/tweets-media?page="+page).success(function (data) {
				for(var i = 0;i<data.length;i++){
					$scope.tweets.push(data[i]);
				}
				$scope.busy = false;
